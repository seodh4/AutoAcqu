...
import json
import cv2
from labelme.TrackingAPI8_0915 import *   
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def ADtoEval(self, _value=False):
        class_list = []
        print(self.lastOpenDir)
        file_list = os.listdir(self.lastOpenDir)
        json_file_list = [file for file in file_list if file.endswith(".json")]
        for json_file in json_file_list:
            with open(self.lastOpenDir + '/' + json_file) as json_file:
                json_data = json.load(json_file)
                for shape in json_data['shapes']:
                    if shape['label'] not in class_list:
                        class_list.append(shape['label'])
        
        self.createFolder(self.lastOpenDir + '/labels/')
        f_w_class = open(self.lastOpenDir + '/labels/classes.txt', 'w')
        for class_label in class_list:
            f_w_class.write(class_label + "\n")
        f_w_class.close()


    def ADtoYOLO(self, _value=False):
        class_list = []
        print(self.lastOpenDir)
        file_list = os.listdir(self.lastOpenDir)
        json_file_list = [file for file in file_list if file.endswith(".json")]
        ...
        for json_file in json_file_list:
            f_w_label = open(self.lastOpenDir + '/labels/' + json_file[:-4]+'.txt', 'w')
            with open(self.lastOpenDir + '/' + json_file) as json_file:
                json_data = json.load(json_file)
                ...
                for shape in json_data['shapes']:
                    ...
                    y_max = float(points[1][1])

                    x_center = float((x_min + x_max)) / 2 / int(imageWidth)
                    y_center = float((y_min + y_max)) / 2 / int(imageHeight)

                    w = float((x_max - x_min)) / int(imageWidth)
                    h = float((y_max - y_min)) / int(imageHeight)

                    r[0] = class_list.index(label)
                    ...
                    f_w_label.write(str(r[0]) +" "+  str(r[1]) +" "+ str(r[2]) +" "+ str(r[3]) +" "+ str(r[4]) + "\n")
    ...
    def get_angle(self,offset_x, offset_y, x,y, angle):
    ...
    def transform_roi(self, fiducial_center, roi, fiducial_center_offset, angle, fiducial_roi, fiducial_roi_flag):
        ...
        # parallel_roi_x1 = roi[0] + fiducial_center_x_differ - fiducial_center_x + fiducial_roi[0]
        # parallel_roi_y1 = roi[1] + fiducial_center_y_differ - fiducial_center_y + fiducial_roi[1]
        # parallel_roi_x2 = roi[0] + roi[2] + fiducial_center_x_differ - fiducial_center_x + fiducial_roi[0]
        # parallel_roi_y2 = roi[1] + fiducial_center_y_differ - fiducial_center_y + fiducial_roi[1]
        # parallel_roi_x3 = roi[0] + roi[2] + fiducial_center_x_differ - fiducial_center_x + fiducial_roi[0]
        # parallel_roi_y3 = roi[1] + roi[3] + fiducial_center_y_differ - fiducial_center_y + fiducial_roi[1]
        # parallel_roi_x4 = roi[0] + fiducial_center_x_differ - fiducial_center_x + fiducial_roi[0]
        # parallel_roi_y4 = roi[1] + roi[3] + fiducial_center_y_differ - fiducial_center_y + fiducial_roi[1]

        transform_roi_x1, transform_roi_y1 = self.get_angle(parallel_roi_x1, parallel_roi_y1, round(angle))
        transform_roi_x2, transform_roi_y2 = self.get_angle(parallel_roi_x2, parallel_roi_y2, round(angle))
        transform_roi_x3, transform_roi_y3 = self.get_angle(parallel_roi_x3, parallel_roi_y3, round(angle))
        transform_roi_x4, transform_roi_y4 = self.get_angle(parallel_roi_x4, parallel_roi_y4, round(angle))
        
        transform_roi_x1 = transform_roi_x1 + fiducial_center_x
        transform_roi_y1 = transform_roi_y1 + fiducial_center_y
        transform_roi_x2 = transform_roi_x2 + fiducial_center_x
        transform_roi_y2 = transform_roi_y2 + fiducial_center_y
        transform_roi_x3 = transform_roi_x3 + fiducial_center_x
        transform_roi_y3 = transform_roi_y3 + fiducial_center_y
        transform_roi_x4 = transform_roi_x4 + fiducial_center_x
        transform_roi_y4 = transform_roi_y4 + fiducial_center_y

        if fiducial_roi_flag:
            transform_roi_x1 = int(int(transform_roi_x1 / 4) * 4) + fiducial_roi[0]
            transform_roi_x2 = int(int(transform_roi_x2 / 4) * 4) + fiducial_roi[0]
            transform_roi_x3 = int(int(transform_roi_x3 / 4) * 4) + fiducial_roi[0]
            transform_roi_x4 = int(int(transform_roi_x4 / 4) * 4) + fiducial_roi[0]
            transform_roi_y1 = int(int(transform_roi_y1 / 4) * 4) + fiducial_roi[1]
            transform_roi_y2 = int(int(transform_roi_y2 / 4) * 4) + fiducial_roi[1]
            transform_roi_y3 = int(int(transform_roi_y3 / 4) * 4) + fiducial_roi[1]
            transform_roi_y4 = int(int(transform_roi_y4 / 4) * 4) + fiducial_roi[1]
        else:
            transform_roi_x1 = int(int(transform_roi_x1 / 4) * 4)
            transform_roi_x2 = int(int(transform_roi_x2 / 4) * 4)
            transform_roi_x3 = int(int(transform_roi_x3 / 4) * 4)
            transform_roi_x4 = int(int(transform_roi_x4 / 4) * 4)
            transform_roi_y1 = int(int(transform_roi_y1 / 4) * 4)
            transform_roi_y2 = int(int(transform_roi_y2 / 4) * 4)
            transform_roi_y3 = int(int(transform_roi_y3 / 4) * 4)
            transform_roi_y4 = int(int(transform_roi_y4 / 4) * 4)
        
        transform_roi = (transform_roi_x1, transform_roi_y1, transform_roi_x2, transform_roi_y2, transform_roi_x3, transform_roi_y3, transform_roi_x4, transform_roi_y4)

        return transform_roi



    def transform_box(self, xyxy, angle, transform_roi_x1, transform_roi_y1):
        ...
        transform_res_x1, transform_res_y1 = self.get_angle(parallel_res_x1, parallel_res_y1, round(angle))
        transform_res_x2, transform_res_y2 = self.get_angle(parallel_res_x2, parallel_res_y2, round(angle))
        transform_res_x3, transform_res_y3 = self.get_angle(parallel_res_x3, parallel_res_y3, round(angle))
        transform_res_x4, transform_res_y4 = self.get_angle(parallel_res_x4, parallel_res_y4, round(angle))
    
        res[0] = transform_res_x1 + transform_roi_x1
        res[1] = transform_res_y1 + transform_roi_y1
        res[2] = transform_res_x2 + transform_roi_x1
        res[3] = transform_res_y2 + transform_roi_y1
        res[4] = transform_res_x3 + transform_roi_x1
        res[5] = transform_res_y3 + transform_roi_y1
        res[6] = transform_res_x4 + transform_roi_x1
        res[7] = transform_res_y4 + transform_roi_y1
    ...
    def AutoLabel(self):
        ...
        shapes = [format_shape(item.shape()) for item in self.labelList]
        imagePath = osp.relpath(self.imagePath, osp.dirname(label_file))
        ...
        master_img=cv2.imread(self.imagePath)
        ...
        fiducial_img = master_img[int(fiducial_points[0][1]):int(fiducial_points[1][1]), int(fiducial_points[0][0]):int(fiducial_points[1][0])]
    

        trainKP, trainDesc, trainImg, im_aspect_ratio, im_height, im_width, imCrop, im_area = search_feature(fiducial_img)
        master_fiducial_center, fiducial_box, angle, check = fiducial_marker(master_img, trainKP, trainDesc, trainImg, im_aspect_ratio, im_height, im_width,im_area)
        ...
        for image_path in self.imageList:
            ...
            if check:
                ...
                if con1 and con2 and con3 and con4:
                    ...
                    imagePath=image_path_s[1],
                    imageData=None,
                    imageHeight=img.shape[0],
                    imageWidth=img.shape[1],)
                
                    with open(self.currentPath()+'/'+image_path_s[1][:-4]+'.json', "w") as f:
                    ...
                    cv2.imwrite(self.currentPath()+'/'+image_path_s[1][:-4]+'_aaa'+'.jpg',img)
