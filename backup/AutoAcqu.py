...
class VideoThread(QThread):
    ...
    def __init__(self,cam_num):
        ...
        # self.caplist = [self.cap_0,self.cap_1,self.cap_2,self.cap_3]
                ...
                if check == False:
                    ...
                    if self.state == 'ready':
                        if contain_condition == True:
                            ...
                            for i,ll in enumerate(self.ll):
                                # cv2.imwrite(self.path + str(self.file_num + 1) +'_'+ 'cam' + str(i) + '.jpg',ll)
...
class WindowClass(QMainWindow, form_class):
    def __init__(self):
        ...
        self.pushButton_run_fi.clicked.connect(self.pushButton_run_fiFunction)
        ...
        self.horizontalSlider_focus_2.valueChanged.connect(self.horizontalSlider_focus_2Function)
        self.spinBox_focus_2.valueChanged.connect(self.spinBox_focus_2Function)
        self.horizontalSlider_exposure_2.valueChanged.connect(self.horizontalSlider_exposure_2Function)
        self.spinBox_exposure_2.valueChanged.connect(self.spinBox_exposure_2Function)
        self.horizontalSlider_brightness_2.valueChanged.connect(self.horizontalSlider_brightness_2Function)
        self.spinBox_brightness_2.valueChanged.connect(self.spinBox_brightness_2Function)
        self.horizontalSlider_contrast_2.valueChanged.connect(self.horizontalSlider_contrast_2Function)
        self.spinBox_contrast_2.valueChanged.connect(self.spinBox_contrast_2Function)
        self.horizontalSlider_saturation_2.valueChanged.connect(self.horizontalSlider_saturation_2Function)
        self.spinBox_saturation_2.valueChanged.connect(self.spinBox_saturation_2Function)
        self.horizontalSlider_sharpness_2.valueChanged.connect(self.horizontalSlider_sharpness_2Function)
        self.spinBox_sharpness_2.valueChanged.connect(self.spinBox_sharpness_2Function)
        self.pushButton_ispinit_2.clicked.connect(self.pushButton_autoip_2Function)


        self.horizontalSlider_focus_3.valueChanged.connect(self.horizontalSlider_focus_3Function)
        self.spinBox_focus_3.valueChanged.connect(self.spinBox_focus_3Function)
        self.horizontalSlider_exposure_3.valueChanged.connect(self.horizontalSlider_exposure_3Function)
        self.spinBox_exposure_3.valueChanged.connect(self.spinBox_exposure_3Function)
        self.horizontalSlider_brightness_3.valueChanged.connect(self.horizontalSlider_brightness_3Function)
        self.spinBox_brightness_3.valueChanged.connect(self.spinBox_brightness_3Function)
        self.horizontalSlider_contrast_3.valueChanged.connect(self.horizontalSlider_contrast_3Function)
        self.spinBox_contrast_3.valueChanged.connect(self.spinBox_contrast_3Function)
        self.horizontalSlider_saturation_3.valueChanged.connect(self.horizontalSlider_saturation_3Function)
        self.spinBox_saturation_3.valueChanged.connect(self.spinBox_saturation_3Function)
        self.horizontalSlider_sharpness_3.valueChanged.connect(self.horizontalSlider_sharpness_3Function)
        self.spinBox_sharpness_3.valueChanged.connect(self.spinBox_sharpness_3Function)
        self.pushButton_ispinit_3.clicked.connect(self.pushButton_autoip_3Function)
        ...
        elif len(self.thread) == 3:
            self.thread[0].change_pixmap_signal.connect(self.update_image0)
            self.thread[1].change_pixmap_signal.connect(self.update_image1)
            self.thread[2].change_pixmap_signal.connect(self.update_image2)
    ...
    def pushButton_run_mnFunction(self):
        if self.thread.trigger_mode == 0:
            ...
            for i,ll in enumerate(self.thread.ll):
                # cv2.imwrite(self.path + str(self.file_num + 1) +'_'+ 'cam' + str(i) + '.jpg',ll)
            
                cv2.imwrite(self.thread.path + 'cam' + str(i) +'_'+ tmstring + '.jpg',ll)

    def pushButton_run_fiFunction(self):
        # img=cv2.imread('./samples/test.jpg')

        if self.thread[0].fiducial_on == False:
            print('Run Inference')
            self.thread[0].fiducial_on = True
            self.pushButton_run_fi.setText("&STOP")
        else:
            self.thread[0].fiducial_on = False
            print('Stop Inference')
            self.pushButton_run_fi.setText("&RUN")
    
    def pushButton_run_fi_1Function(self):
        # img=cv2.imread('./samples/test.jpg')

        if self.thread[1].fiducial_on == False:
            print('Run Inference')
            self.thread[1].fiducial_on = True
            self.pushButton_run_fi_1.setText("&STOP")
        else:
            self.thread[1].fiducial_on = False
            print('Stop Inference')
    ...
    def fiducial_signal_0(self, thread):
        ...
        self.label_screen_fiducial.setPixmap(imcrop_qt_img)
    ...
    def pushButton_autoip_2Function(self):
        self.current_cap = 2
        self.focus_2 = int(self.thread[self.current_cap].cap.get(cv2.CAP_PROP_FOCUS))
        self.brightness_2 = int(self.thread[self.current_cap].cap.get(cv2.CAP_PROP_BRIGHTNESS))
        self.contrast_2 = int(self.thread[self.current_cap].cap.get(cv2.CAP_PROP_CONTRAST))
        self.saturation_2 = int(self.thread[self.current_cap].cap.get(cv2.CAP_PROP_SATURATION))
        self.exposure_2 = int(self.thread[self.current_cap].cap.get(cv2.CAP_PROP_EXPOSURE))
        self.sharpness_2 = int(self.thread[self.current_cap].cap.get(cv2.CAP_PROP_SHARPNESS))

        print('focus : ' + str(self.focus_2))
        print('brightness : ' + str(self.brightness_2))
        print('contrast : ' + str(self.contrast_2))
        print('saturation : ' + str(self.saturation_2))
        print('exposure : ' + str(self.exposure_2))
        print('sharpness : ' + str(self.sharpness_2))
    ...
    def horizontalSlider_focus_2Function(self):
        self.current_cap = 2
        self.spinBox_focus_2.setValue(self.horizontalSlider_focus_2.value())
        self.focus_2 = self.horizontalSlider_focus_2.value() *5
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_AUTOFOCUS, 0)
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_FOCUS, self.focus_2)

    def spinBox_focus_2Function(self):
        self.horizontalSlider_focus_2.setValue(self.spinBox_focus.value())

    #-- CAP_PROP_BRIGHTNESS, 0~255, step=1, default=128
    def horizontalSlider_brightness_2Function(self):
        self.current_cap = 2
        self.spinBox_brightness_2.setValue(self.horizontalSlider_brightness_2.value())
        self.brightness_2 = self.horizontalSlider_brightness_2.value()
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_BRIGHTNESS, self.brightness_2/1)

    def spinBox_brightness_2Function(self):
        self.horizontalSlider_brightness_2.setValue(
            self.spinBox_brightness_2.value())

    #-- CAP_PROP_CONTRAST, 0~255, step=1
    def horizontalSlider_contrast_2Function(self):
        self.current_cap = 2
        self.spinBox_contrast_2.setValue(self.horizontalSlider_contrast_2.value())
        self.contrast_2 = self.horizontalSlider_contrast_2.value()
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_CONTRAST, self.contrast_2/1)

    def spinBox_contrast_2Function(self):
        self.horizontalSlider_contrast_2.setValue(self.spinBox_contrast_2.value())

    #-- CAP_PROP_SATURATION, 0~255, step=1
    def horizontalSlider_saturation_2Function(self):
        self.current_cap = 2
        self.spinBox_saturation_2.setValue(
            self.horizontalSlider_saturation_2.value())
        self.saturation_2 = self.horizontalSlider_saturation_2.value()
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_SATURATION, self.saturation_2/1)

    def spinBox_saturation_2Function(self):
        self.horizontalSlider_saturation_2.setValue(
            self.spinBox_saturation_2.value())

    #-- CAP_PROP_EXPOSURE, 3~2047, step=1, default=250
    def horizontalSlider_exposure_2Function(self):
        self.current_cap = 2
        self.spinBox_exposure_2.setValue(self.horizontalSlider_exposure_2.value())
        self.exposure_2 = self.horizontalSlider_exposure_2.value()
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 0)
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_EXPOSURE, self.exposure_2)

    def spinBox_exposure_2Function(self):
        self.horizontalSlider_exposure_2.setValue(self.spinBox_exposure_2.value())

    #-- CAP_PROP_SHARPNESS, 0~255, step=1, default=128
    def horizontalSlider_sharpness_2Function(self):
        self.current_cap = 2
        self.spinBox_sharpness_2.setValue(
            self.horizontalSlider_sharpness_2.value())
        self.sharpness_2 = self.horizontalSlider_sharpness_2.value()
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_SHARPNESS, self.sharpness_2/1)

    def spinBox_sharpness_2Function(self): 
        self.horizontalSlider_sharpness_2.setValue(
            self.spinBox_sharpness_2.value())




    def pushButton_autoip_3Function(self):
        self.current_cap = 3
        self.focus_3 = int(self.thread[self.current_cap].cap.get(cv2.CAP_PROP_FOCUS))
        self.brightness_3 = int(self.thread[self.current_cap].cap.get(cv2.CAP_PROP_BRIGHTNESS))
        self.contrast_3 = int(self.thread[self.current_cap].cap.get(cv2.CAP_PROP_CONTRAST))
        self.saturation_3 = int(self.thread[self.current_cap].cap.get(cv2.CAP_PROP_SATURATION))
        self.exposure_3 = int(self.thread[self.current_cap].cap.get(cv2.CAP_PROP_EXPOSURE))
        self.sharpness_3 = int(self.thread[self.current_cap].cap.get(cv2.CAP_PROP_SHARPNESS))

        print('focus : ' + str(self.focus_3))
        print('brightness : ' + str(self.brightness_3))
        print('contrast : ' + str(self.contrast_3))
        print('saturation : ' + str(self.saturation_3))
        print('exposure : ' + str(self.exposure_3))
        print('sharpness : ' + str(self.sharpness_3))
    ...
    def horizontalSlider_focus_3Function(self):
        self.current_cap = 3
        self.spinBox_focus_3.setValue(self.horizontalSlider_focus_3.value())
        self.focus_3 = self.horizontalSlider_focus_3.value() *5
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_AUTOFOCUS, 0)
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_FOCUS, self.focus_3)

    def spinBox_focus_3Function(self):
        self.horizontalSlider_focus_3.setValue(self.spinBox_focus.value())

    #-- CAP_PROP_BRIGHTNESS, 0~255, step=1, default=128
    def horizontalSlider_brightness_3Function(self):
        self.current_cap = 3
        self.spinBox_brightness_3.setValue(self.horizontalSlider_brightness_3.value())
        self.brightness_3 = self.horizontalSlider_brightness_3.value()
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_BRIGHTNESS, self.brightness_3/1)

    def spinBox_brightness_3Function(self):
        self.horizontalSlider_brightness_3.setValue(
            self.spinBox_brightness_3.value())

    #-- CAP_PROP_CONTRAST, 0~255, step=1
    def horizontalSlider_contrast_3Function(self):
        self.current_cap = 3
        self.spinBox_contrast_3.setValue(self.horizontalSlider_contrast_3.value())
        self.contrast_3 = self.horizontalSlider_contrast_3.value()
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_CONTRAST, self.contrast_3/1)

    def spinBox_contrast_3Function(self):
        self.horizontalSlider_contrast_3.setValue(self.spinBox_contrast_3.value())

    #-- CAP_PROP_SATURATION, 0~255, step=1
    def horizontalSlider_saturation_3Function(self):
        self.current_cap = 3
        self.spinBox_saturation_3.setValue(
            self.horizontalSlider_saturation_3.value())
        self.saturation_3 = self.horizontalSlider_saturation_3.value()
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_SATURATION, self.saturation_3/1)

    def spinBox_saturation_3Function(self):
        self.horizontalSlider_saturation_3.setValue(
            self.spinBox_saturation_3.value())
    #-- CAP_PROP_EXPOSURE, 3~2047, step=1, default=250
    def horizontalSlider_exposure_3Function(self):
        self.current_cap = 3
        self.spinBox_exposure_3.setValue(self.horizontalSlider_exposure_3.value())
        self.exposure_3 = self.horizontalSlider_exposure_3.value()
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 0)
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_EXPOSURE, self.exposure_3)

    def spinBox_exposure_3Function(self):
        self.horizontalSlider_exposure_3.setValue(self.spinBox_exposure_3.value())
    #-- CAP_PROP_SHARPNESS, 0~255, step=1, default=128
    def horizontalSlider_sharpness_3Function(self):
        self.current_cap = 3
        self.spinBox_sharpness_3.setValue(
            self.horizontalSlider_sharpness_3.value())
        self.sharpness_3 = self.horizontalSlider_sharpness_3.value()
        self.thread[self.current_cap].cap.set(cv2.CAP_PROP_SHARPNESS, self.sharpness_3/1)
    def spinBox_sharpness_3Function(self): 
        self.horizontalSlider_sharpness_3.setValue(
            self.spinBox_sharpness_3.value())
    ...
    def update_image2(self, img):
        ...
        if self.canvas_mode0 != 'fiducial' and self.canvas_mode0 != 'roi':
            self.temp_canvas = img.copy()
            qt_img0 = self.convert_cv_qt(self.thread[2].cv_img,640,360)
            self.label_screen_2.setPixmap(qt_img0)

        # elif self.canvas_mode0 == 'fiducial':
        #     if self.state_draw_rect:
        #         # cv2.rectangle(self.temp_canvas,(self.past_x*2,self.past_y*2),(self.present_x*2,self.present_y*2),(0,255,0),1)
        #         qt_img0 = self.convert_cv_qt(self.temp_canvas,640,360)
        #         self.draw_rect(qt_img0,self.past_x,self.past_y,self.present_x,self.present_y,Qt.green)
        #         self.label_screen_2.setPixmap(qt_img0)
        # elif self.canvas_mode0 == 'roi':
        #     if self.state_draw_rect:
